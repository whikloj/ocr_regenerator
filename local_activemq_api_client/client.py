...
class ActiveMQClient:
        ...
        for queue in tree.findall('queue'):
            ...
            if name is not None:
                details = {
                    ...
                    'size': int(queue.find('enqueueCount').text) - int(queue.find('dequeueCount').text)
                }
            else:
                name = queue.attrib.get('name')
                stats = queue.find('stats')
                details = {
                    'name': name,
                    'consumerCount': int(stats.attrib.get('consumerCount')),
                    'enqueueCount': int(stats.attrib.get('enqueueCount')),
                    'dequeueCount': int(stats.attrib.get('dequeueCount')),
                    'size': int(stats.attrib.get('size'))
                }
    ...
    def get_queue_size(self, queue_name: str) -> Union[int, None]:
        queues = self.get_queues_details()
        for queue in queues:
            if queue['name'] == queue_name:
                return queue['size']
